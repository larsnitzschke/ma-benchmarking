...
for (example_name, mode) in aggregated_results:
    ...
    if True: #aggregated_results[(example_name, mode)]['Classification'] == "":
        ...
        if aggregated_results[(example_name, mode)]['Safe'] == "OUTOFMEMORY" \
                and mode != "-gB --gpdr-smi":
            print(f"Timeout result for {example_name} in mode {mode}: {aggregated_results[(example_name, mode)]['Safe']}")
...
def results_by_approach_for_metric(aggregated_results, metric, complexity = False):
    ...
    for (example_name, mode) in aggregated_results:
        ...
        if aggregated_results[(example_name, mode)]['Safe'] != "Proof" and aggregated_results[(example_name, mode)]['Safe'] != "Counterexample" and aggregated_results[(example_name, mode)]['Safe'] != "NoResult":
            continue
        if aggregated_results[(example_name, "-gB --gpdr-ats")]['Safe'] != "Proof" and aggregated_results[(example_name, "-gB --gpdr-ats")]['Safe'] != "Counterexample" and aggregated_results[(example_name, "-gB --gpdr-ats")]['Safe'] != "NoResult":
            continue
    ...
    results["bmc"] = [a[1] for a in sorted(bmc_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["kInd"] = [a[1] for a in sorted(kInd_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["kInd_inv"] = [a[1] for a in sorted(kInd_inv_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["bmc_kInd"] = [a[1] for a in sorted(bmc_kInd_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["bmc_kInd_inv"] = [a[1] for a in sorted(bmc_kInd_inv_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["wpc"] = [a[1] for a in sorted(wpc_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]

    results["gpdr"] = [a[1] for a in sorted(gpdr_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["gpdr_boolEval"] = [a[1] for a in sorted(gpdr_boolEval_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["gpdr_ats"] = [a[1] for a in sorted(gpdr_ats_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["gpdr_ats_boolEval"] = [a[1] for a in sorted(gpdr_ats_boolEval_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["gpdr_smi"] = [a[1] for a in sorted(gpdr_smi_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["gpdr_smi_boolEval"] = [a[1] for a in sorted(gpdr_smi_boolEval_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["gpdr_smi_ats"] = [a[1] for a in sorted(gpdr_smi_ats_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    results["gpdr_smi_ats_boolEval"] = [a[1] for a in sorted(gpdr_smi_ats_boolEval_results.items(), key= lambda item: gpdr_ats_boolEval_results.get(item[0]))]
    return results
